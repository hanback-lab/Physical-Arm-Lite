...
1. 토큰 위로 팔을 이동한다.
2. 그리퍼를 열고 토큰을 집는다.
2-1. 토큰이 빨간색인 경우 왼쪽 상자로 이동한다.
2-2. 토큰이 초록색인 경우 오른쪽 상자로 이동한다.
3. 상자 안에 토큰을 넣는다.
4. 초기 상태로 만든 뒤 반복한다.
...
한 번에 여러 행동을 빠르게 수행하기보다는, 조금씩 끊어서 천천히 조작하는 것을 권장합니다. 성공한 에피소드는 `s`, 실패한 에피소드는 `f`를 입력합니다. 다음 에피소드를 시작하려면 `start`를 입력합니다. 이 과정을 약 10회 정도 반복합니다. 데이터 수집이 끝났다면 `q`를 입력하여 노드를 종료합니다. 나머지 런치 파일과 노드도 `Ctrl+C`로 종료합니다. 같은 위치만 집는 학습을 시킨다면 빨간 토큰 10개, 초록 토큰 10개씩 총 20개의 데이터만으로도 충분합니다. 다양한 위치에서 집어서 분류하도록 학습시키기 위해서는 더 많은 데이터 수집이 필요합니다. 두 개를 두고 순차적으로 분류하는 작업은 최소 50개 정도의 데이터셋이 필요합니다.

데이터 수집이 완료되면 parsing을 위해 데이터를 구글 드라이브로 옮겨야 합니다. dataset_raw_classify 폴더가 있는 경로를 찾아 이동한 뒤, 압축해 줍니다.

```sh
zip -r dataset_raw_classify.zip dataset_raw_classify
```
...
로그인을 진행한 후, 압축을 푼 원시 데이터 파일을 이용해 parsing을 진행하겠습니다.
...
RAW_ROOT = Path("/content/dataset_raw_classify")
OUT_REPO_ID = "<id>/classify_tokens_dataset"
...
```py
!lerobot-train \
  --dataset.repo_id=<id>/classify_tokens_dataset \
  --policy.type=act \
  ...
  --batch_size=16 \
  --job_name=classify \
  --policy.repo_id=<id>/classify_tokens \
  --policy.device=cuda \
  --wandb.enable=false
```
...
upload_folder(
    repo_id="<id>/classify_tokens",
    repo_type="model",
    folder_path="outputs/train/classify_tokens/checkpoints/last/pretrained_model",
)
```
...
ds = LeRobotDataset("<id>/classify_tokens_dataset")
...
with open("dataset_stats_classification.json", "w") as f:
    json.dump(stats, f)

print(stats)
```

dataset_stats_classification.json 파일을 구글 드라이브에 업로드 해줍니다. 그 후, Jetson 환경으로 돌아와 업로드했던 파일을 `/home/soda/physicai_arm_ws/src/physicai_arm/physicai_arm` 폴더에 복사해 줍니다.

Jetson 환경에 LeRobot이 다운되어 있지 않다면 아래 LeRobot 설치를 진행해주시기 바랍니다.
