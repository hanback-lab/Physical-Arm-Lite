...
실제 장비와 함께 실습하는 경우 Physical Arm Lite에 연결한 뒤, 다음 명령어를 입력하여 패키지를 생성합니다.
...
Physical Arm Lite 장비에서 각 링크의 위치와 좌표계 관계를 계산하기 위해서는 동일하게 로봇 모델링 파일이 필요합니다. Physical Arm Lite의 각 링크 및 조인트의 정보는 아래와 같습니다.
...
위 정보들을 바탕으로 URDF를 작성하여 로봇 모델링 파일을 만들고 TF를 발행합니다. 원활한 실습을 위해 모델링 진행 및 TF 발행, 그리고 다음 챕터에서 소개할 제어 토픽까지 모두 포함된 Physical Arm Lite 제어 패키지를 제공합니다.
...
- 본 챕터의 목차 *Physical Arm Lite 모델링 정보*를 참고하여 제공된 `physicai_arm` 패키지를 준비한 후 복사하고, 임의의 이름으로 변경하여 작업을 진행하시오.
