...
3장 기초 로봇공학에서 좌표계가 무엇이고 각 조인트마다 좌표계가 부착되며 특정 연산을 통해 매니퓰레이터의 위치와 방향이 바뀔 수 있다고 배웠습니다. 그렇다면 이를 실전 로봇 제어에 도입하려면 어떻게 해야 할까요? 로봇을 제어할 때마다 행렬 연산을 하는 건 매우 번거로운 일입니다.
