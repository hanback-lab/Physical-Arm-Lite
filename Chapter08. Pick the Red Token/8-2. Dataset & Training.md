...
한 번에 여러 행동을 빠르게 수행하기보다는, 조금씩 끊어서 천천히 조작하는 것을 권장합니다. 성공한 에피소드는 `s`, 실패한 에피소드는 `f`를 입력합니다. 다음 에피소드를 시작하려면 `start`를 입력합니다. 이 과정을 약 10회 정도 반복합니다. 데이터 수집이 끝났다면 `q`를 입력하여 노드를 종료합니다. 나머지 런치 파일과 노드도 `Ctrl+C`로 종료합니다. 같은 위치만 집는 학습을 시킨다면 10개의 데이터만으로도 충분합니다. 여러 위치에서 집도록 학습할 경우 더 많은 데이터 수집이 필요합니다.

데이터 수집이 완료되면 parsing을 위해 데이터를 구글 드라이브로 옮겨야 합니다. dataset_raw 폴더가 있는 경로를 찾아 이동한 뒤, 압축해 줍니다.
...
데이터 파싱은 Colab에서 진행하겠습니다. [7-3. Google Colab](/Chapter07.%20AI%20Overview/7-3.%20Google%20Colab.md)을 참고하여 Google Colab을 열어줍니다. 본격적으로 시작하기에 앞서, 구글 드라이브 마운트를 진행하겠습니다.
...
데이터 parsing을 하기 위해선 'lerobot'과 'huggingface' 패키지가 필요합니다. Colab에 해당 패키지를 설치하겠습니다.
...
설치한 후, huggingface에 업로드하기 위한 계정에 로그인하겠습니다. 보안상의 이유로 colab의 secret key 기능을 사용하겠습니다. 아래와 같이 클릭하여 secret 섹션으로 들어가 주시기 바랍니다. Name에는 `HF_TOKEN`을 입력해 주고 Value에는 huggingface에서 발급받은 API key를 입력하시기 바랍니다.


<img src="pds/secret_key.png" style="width:60%;">
...
RAW_ROOT = Path("/content/dataset_raw/dataset_raw")
...
!lerobot-train \
  ...
  --policy.repo_id=<id>/act-red-token \
  --policy.device=cuda \
  --wandb.enable=false
```
