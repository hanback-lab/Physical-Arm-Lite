...
학습된 checkpoint/model이 준비되었다면 생성된 모델을 이용하여 로봇을 제어해보겠습니다. 로봇이 실행 중 과하게 움직이거나 멈추고 싶을 때에는 `inference_node`를 실행 중인 터미널에서 `Ctrl+C`를 눌러 노드 실행을 중단하십시오.

봇봇을 실행하기 위해서는 첫 번째 터미널에서 bringup launch를 실행하고, 두 번째 터미널에서 추론 노드를 실행해야 합니다. 로봇이 먼저 시작될 수 있도록 순서대로 각 터미널에 아래 명령어를 입력해 주십시오.
