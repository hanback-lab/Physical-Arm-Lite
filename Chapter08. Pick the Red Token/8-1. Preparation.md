...
LeRobot을 단독으로 사용할 때와 달리, ROS2를 통해 LeRobot을 사용하면 성능이 급격히 저하되는 것을 확인할 수 있습니다. 이에 따른 원인은 타이밍 불일치, 좌표계의 변환, 버퍼링으로 인한 지연, 데이터 오염, 설정 불일치 등으로 다양합니다. 데이터의 타이밍을 맞추기 위해 저장 시 공통 기준의 타임스탬프를 남기고, 이후 파싱 단계에서 시간 기준으로 데이터를 정렬하겠습니다. 하나의 타임스탬프 안에 4개의 데이터가 묶이기 때문에 매칭 오차가 발생하지 않습니다.
...
Lerobot은 학습할 때 **정규화와 역정규화**를 이용합니다. 학습에서만 정규화를 진행하고 추론에서 진행하지 않으면 결과가 정확하지 않게 출력됩니다. 한쪽으로만 가거나 갈팡질팡하는 모습을 볼 수 있습니다. 따라서 추론 노드에 정규화와 역정규화 과정을 추가합니다. 정규화를 위해 최초 실행 때 데이터 전체를 읽고 통계값을 파일로 저장합니다. 이후 추론에서는 저장된 파일을 읽고 정규화를 진행합니다.
...
CALIB_PATH = ""
STATS_CACHE_PATH = "dogeum/pick_red_token"
...
class InferenceSimNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("policy_path", "/home/soda/.cache/huggingface/hub/models--dogeum--act-stacking-tokens/snapshots/7c5510956b31f060c0cb410f8d920f282186fc55")
