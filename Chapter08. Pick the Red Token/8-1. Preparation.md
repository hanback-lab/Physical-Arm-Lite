...
실습에는 총 3개의 노드가 필요합니다. 데이터를 저장하는 노드, 저장된 원시 데이터를 LeRobot 형식에 맞게 파싱하는 노드, 추론 노드입니다. 각 노드의 이름은 logger_node.py, parsing_node.py, inference_node.py로 하겠습니다. 학습은 별도 ROS2 노드가 아니라 LeRobot의 `lerobot-train` 명령으로 진행합니다. 먼저 logger_node부터 작성하겠습니다.
...
Lerobot은 학습할 때 정규화와 역정규화를 이용합니다. 학습에서만 정규화를 진행하고 추론에서 진행하지 않으면 결과가 이상하게 출력됩니다. 한쪽으로만 가거나 갈팡질팡하는 모습을 볼 수 있습니다. 따라서 이번 추론 노드에서도 정규화와 역정규화 과정을 추가합니다. 정규화를 위해 최초 실행 때 데이터 전체를 읽고 통계값을 파일로 저장합니다. 이후는 저장된 파일을 읽고 정규화를 진행합니다.
