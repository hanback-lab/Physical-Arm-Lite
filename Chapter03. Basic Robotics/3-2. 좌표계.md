...
한 공간에 좌표계 $A$, $B$ 가 존재한다고 가정하겠습니다. 이 둘 사이의 관계를 표현하려면 어떻게 해야할까요? 만약 좌표계 $A$ 에서 표시된 점 $P$ 를 좌표계 $B$ 에서 표현하려고 할려면, 어떤 연산을 시켜주어야 정확하게 표시할 수 있을까요?
...
예를 들어 $^{A}t_{B} = \begin{bmatrix} 2 & 1 & 0 \end{bmatrix}^{T}$ 라면, 이는 '$B$의 원점이 $A$ 에서 볼 때 $x$ 방향으로 2, $y$ 방향으로 1만큼 떨어져 있다' 라는 뜻을 가지고 있습니다. 이 때 아직 축 방향은 결정짓지 않았기 때문에, 좌표계의 위치 벡터만 담아 냅니다.
