...
순기구학(Forward Kinematics)이란 링크의 두 부분 사이 좌표계 관계를 그 링크의 관절 값(joint values)에 기초하여 수학적으로 결정하는 것을 의미합니다. 그리고 매니퓰레이터의 경우 Base 좌표계와 EE 좌표계 사이의 관계를 구하는 문제입니다.
...
순기구학의 원리는 각 조인트의 앞뒤 좌표계 사이에 하나의 로컬 변환(local transform)을 만드는 것입니다. 이 로컬 변환은 앞에서 배운 동차 변환 행렬로 표현합니다. 그러면 base에서부터 첫 번째 링크, 첫 번째 링크에서 두 번째 링크, 두 번째 링크에서 n번째 링크처럼 기구학 체인(kinematic chain)을 따라 변환을 차례대로 곱해 나가면 최종적으로 EE의 자세가 계산됩니다.
...
### 실습 1 : 2-link 로봇의 EE 자세 파악하기

아래는 관절 각이 주어졌을 때, 각 링크의 위치와 최종 EE 자세를 계산하여 이를 시각화하는 코드입니다. 조인트는 Revolute Joint를 사용합니다.
...
Matplotlib Slider를 통해 관절 각을 바꿔보도록 하겠습니다.
...
앞에서 설명한 순기구학은 관절 값을 알고 있을 때 EE 자세를 구하는 과정입니다. 그리고 원하는 EE 자세로부터 관절 값을 구하는 것이 역기구학(Inverse Kinematics)입니다. 역기구학은 링크의 두 부분 사이 좌표계 관계를 바탕으로, 조인트의 관절 값을 수학적으로 결정하는 것입니다. 매니퓰레이터의 경우 보통 Base 좌표계와 EE 좌표계 사이의 관계로부터 관절 값을 구하는 것입니다.
...
### 실습: 2-link 로봇의 EE 위치에 따른 관절 값 구하기

아래는 EE 위치가 주어졌을 때, 그 위치에 도달할 수 있는 관절 각을 계산하여 이를 시각화한 코드입니다. 조인트는 Revolute Joint를 사용합니다.
...
두 조인트와 목표점을 꼭짓점으로 보면 하나의 삼각형을 만들 수 있습니다. 이를 이용해 코사인 법칙을 활용하여 두 번째 관절 각 $\theta_{2}$에 대한 코사인 값을 알 수 있습니다.
...
코사인 값을 알아냈다면, 두 번째 관절 각을 계산합니다. $\sin \theta_{2}$와 $\cos \theta_{2}$를 이용해 $\theta_{2}$를 복원합니다.
...
IK에는 항상 해가 있는 것이 아니며 workspace의 제한이 있다는 점을 유의하며 실습을 진행해 보겠습니다. 최대 도달 거리와 최소 도달 거리를 알아보고 목표점까지의 거리가 이 범위 안에 있어야 IK 해가 존재한다는 점을 알아보겠습니다.
...
FK는 관절 값에서 EE 값을 구하고, IK는 원하는 EE 자세에서 관절 값을 구하는 개념입니다. 이 둘을 연결하는 심화학습 예제입니다. 단순히 IK로 관절 각을 구하는 데서 끝나지 않습니다. IK로 얻은 관절 각을 다시 FK로 넣어 실제 EE 위치를 계산하고 목표점과의 오차를 확인하겠습니다.
...
2. 같은 목표점에서 관절 각이 어떻게 달라지는지 비교하시오.
...
4. 2-link 로봇에서 l1=2, l2=2 이 있을 때, 목표점이 (2,2)이면 가능한 IK 해는 몇 개인가?
