...
환경 설정 충돌을 방지하기 위해 가상환경을 사용하여 다운로드하겠습니다.
...
`/home/soda/lerobot_infer_env/bin/python` 라는 메시지가 나오면 성공입니다. 다르게 뜨는 경우 위에서 source를 한번 더 실행해주세요. 가상환경이 설정되었으면 MuJoCo를 설치하겠습니다.
...
MuJoCo는 로봇의 움직임을 물리적으로 계산하는 시뮬레이터입니다. 로봇의 관절 구조, 질량, 관성, 충돌 정보 등을 기반으로 실제 환경과 유사한 움직임을 계산할 수 있습니다. 하지만 MuJoCo 자체는 로봇 시스템 전체를 구상하기 위한 프레임워크가 아니므로 센서 처리, 장치 제어, 알고리즘 실행과 같은 기능은 제공하지 않습니다.
...
ROS2와 MuJoCo가 함께 동작할 때 가장 중요한 것은 데이터의 흐름입니다. 로봇 제어 프로그램은 ROS2 노드로 구현되며, 제어 명령은 **topic을 통해 전달**됩니다. MuJoCo는 해당 명령을 받아 물리 시뮬레이션을 수행하고, 계산된 결과를 다시 ROS2 토픽으로 전송합니다.

예를 들어 로봇 팔의 첫 번째 관절을 30도 회전시키고자 할 경우 다음과 같은 과정이 이루어집니다.

1. ROS2 제어 노드가 목표 관절각을 생성한다.
2. 목표 관절각이 topic을 통해 MuJoCo 노드로 전달된다.
3. MuJoCo는 관절의 움직임을 물리적으로 계산한다.
4. 계산된 관절 위치와 속도를 ROS2 topic으로 발행한다.
...
|시뮬레이터|특징|장점|단점|
|--------|---|---|---|
|MuJoCo|물리 엔진 중심의 로봇 시뮬레이터|물리 계산이 빠르고 정확하며 코드로 제어하기 쉬움|복잡한 환경 구성은 직접 구현해야함
|Gazebo|ROS와 함꼐 많이 사용되는 범용 로봇 시뮬레이터|ROS2와 연동성이 좋고 실제 로봇 개발 흐름과 비슷하게 테스트 가능|충돌/접촉 계산이 튀는 경우가 있음|
|Isaac Sim|NVIDIA Omniverse 기반의 고성능 로봇 시뮬레이터|디지털 트윈에 강하고 GPU 가속을 활용할 수 있음|고사양 GPU가 필요하며 설치 용량과 실행 환경이 무거움|


### 실습 : MuJoCo 중력

실습을 진행하기 위해선 환경구성이 필요합니다. 실행한 가상환경에서 ros2 환경을 불러오도록 하겠습니다.
...
기본적인 MuJoCo 예제는 XML 하나 + Python 하나 입니다. 
...
작성한 파일을 저장한 후, python3를 이용해 파일을 실행해보겠습니다. 물리작용으로 인해 막대기가 흔들리는 것을 확인할 수 있습니다.
...
PhysicaAI Arm을 MuJoCo로 불러와 동기화시키겠습니다. 파일은 `physicai_arm.xml`을 이용합니다.
...
IKPy는 로봇팔의 순기구학과 역기구학을 계산해주는 파이썬 라이브러리입니다. 로봇을 움직일 때는 보통 두 가지 방법이 있습니다. 관절각을 직접 지정하거나 EE 위치를 지정하는 방법입니다. EE 위치를 지정하면 관절값들을 계산해야하는데, 이를 계산하는 것이 역기구학입니다. 순기구학은 관절각이 정해졌을 때 EE의 위치가 어디인지 계산하는 것입니다. 
...
목표 EE 위치를 입력하면 IKPy가 역기구학을 이용해 관절각을 계산하고 이를 MuJoCo에 넣는 시뮬레이션을 진행하겠습니다. 직접 EE의 위치를 정한 다음 MuJoCo에서 로봇팔의 움직임을 확인해보겠습니다.
...
이번 실습에서는 로봇 모델링 파일을 가져온 후, 실제 로봇은 움직이지 않고 시뮬레이터 속 로봇만 제어해보도록 하겠습니다. 기존의 joystick launch 파일을 이용하여 topic을 발행하고, 해당 topic을 구독하여 시뮬레이터 속 로봇을 움직이겠습니다.

bringup launch 파일은 실행하지 않습니다.
...
작성한 파일을 실행한 후, `joystick.launch.py` 런치 파일을 실행해주면 조이스틱을 이용해 시뮬레이터 속 로봇이 제어되는 것을 확인할 수 있습니다. `/joint_states` topic을 통해 제어되므로, 해당 토픽을 발행하여 확인할 수 있습니다.

bringup 런치 파일과 joystick 런치 파일을 같이 실행시키면 PhysicAI Arm을 제어하며 실제 로봇과 시뮬레이션 속 로봇이 동기화된 것을 확인할 수 있습니다.

해당 실습까지만 직접 실행으로 진행하고 다음 실습부터는 ros2 run을 이용해 진행합니다. 해당 실습 파일은 다음 실습을 진행하는 동안에도 켜두어야 정상적으로 작동됩니다.
...
다음으로는 시뮬레이터에서 제어되는 로봇의 데이터를 저장해보도록 하겠습니다. 이는 추후 'Pick the Red token' 등 Imitation Learning에 나오는 `logger_node`와 유사하게 구성됩니다. 사용되는 topic이 같으므로 수월하게 진행할 수 있습니다.

이번 노드에서는 `logger_node`와는 달리 `/joint_states` topic만을 timestamp와 함께 저장하겠습니다. 또한 이미지나 기타 다른 데이터는 수집하지 않고, episode 형식으로 구성되지 않았습니다.

이번 실습을 진행하기 위해서는 위 `joystick.py` 예제가 실행중인 상태여야 합니다. 이번 실습부터는 터미널을 하나 더 열고, physicai_arm 패키지로 돌아가서 node를 이용해 실행하겠습니다. 먼저 패키지로 들어간 뒤 ros환경 설정을 위해 소싱을 진행합니다. 가상환경은 사용하지 않습니다.


```sh
cd ~/physicai_arm_ws/physicai_arm
source install/setup.bash
```
...
노드를 실행한 뒤, 조이스틱을 통해 로봇을 움직이고 `Ctrl+C`를 입력하여 종료합니다. 기록이 종료되면 'joint_states.csv' 파일이 생긴 것을 확인할 수 있습니다. 해당 파일은 아래와 같이 joint의 관절값이 기록되어 있습니다.

<img src="pds/joint.png">

## 실제 로봇과 시뮬레이터 로봇 비교

동일한 목표 관절각을 입력하였을 때 실제 로봇과 MuJoCo 시뮬레이터가 얼마나 정확하게 목표 위치를 추종하는지 비교하겠습니다. 실제 로봇은 모터 제어 오차, 기구적 오차, 통신 지연 등의 영향을 받는 반면, MuJoCo는 이상적인 물리 모델을 사용하기 때문에 보다 정확하게 목표 자세를 재현할 수 있습니다.

본 실습을 통해 실제 환경과 시뮬레이션 환경의 차이를 이해하고 시뮬레이터를 이용한 알고리즘 검증의 장점을 확인하겠습니다.

조이스틱을 이용하여 로봇을 제어하고, 생성된 `/joint_states`는 실제 로봇과 MuJoCo에 동일하게 입력됩니다. 실험 중 두 환경의 `/joint_states`를 각각 저장하고 비교합니다.
...
노드를 작성한 후, `setup.py`에 등록한 뒤 colcon build를 해줍니다. 해당되는 런치 파일과 노드(`bringup.launch.py`, `joystick.launch.py`, `<MuJoCo실행파일>`, `robot_graph_node.py`)들을 실행하면 화면에 각각 관절값이 그려지는 것을 확인할 수 있습니다. 원하는 관절값을 체크하면 해당 값을 확인할 수 있습니다.
...
`urdf2mjcf`는 Benjamin Bolte가 만든 패키지로, Python3.11+가 필요하며 현재 활발히 업데이트 중입니다. 
actuator, joint limit, mesh convex hull 처리를 자동으로 해줘서 실제 로봇 URDF에 적합합니다.

**방법 3 : ROS2 연동 변환**

ROS2 환경에서는 `/robot_description` topic에서 직접 URDF를 받아 변환하는 스크립트를 제공합니다. mesh를 `assets/` 폴더로 정리하고 convex hull 분해까지 자동으로 수행합니다. ROS2 통합이라면 이 방식이 가장 통합적입니다.
...
|Feetech 서보 actuator|MJCF에서 `<acuator>`태그 수동 추가 필요|
...
5. PhysicAI Arm 디지털 트윈 코드에서 `/joint_states` topic을 구독하는 이유는 무엇인가?
