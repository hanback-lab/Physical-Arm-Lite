...
이번 장에서는 저장 폴더를 `stacking_dataset_raw`, task name을 `stacking_token`으로 사용합니다.
...
class LoggerNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("Task: stack green token on red token")
    ...
    def save_metadata(self, success=None, fail_reason=None):
        metadata = {
            ...
            "task_rule": "pick red token, place it in the center, then stack green token on it",
...
`parsing_node`는 `logger_node`가 저장한 `stacking_dataset_raw` 폴더의 원시 데이터를 읽어 LeRobot이 학습할 수 있는 데이터셋 형식으로 변환합니다. 실패한 에피소드는 제외하고, timestamp를 기준으로 이미지, state, action을 정렬합니다.

이번 장에서는 출력 데이터셋 repo id를 `local/stacking_token_dataset`으로 사용합니다.
...
이번 장에서는 학습 결과 경로를 `outputs/train/stacking_token/checkpoints/last/pretrained_model`, 정규화 통계 파일을 `stacking_dataset_stats.json`으로 사용합니다.
...
class InferenceSimNode(Node):
    def __init__(self):
        ...
        stats = load_or_compute_stats("local/stacking_token_dataset", STATS_CACHE_PATH)
