...
- `local/stacking_token_dataset` 데이터셋으로 저장합니다.
...
데이터가 준비되면 LeRobot을 활용하여 ACT policy를 학습합니다. 이번 예제의 입력 데이터 위치와 출력 위치는 `stacking_token` 기준으로 설정합니다.

- 배치 사이즈(batch_size): 8
- 스텝 수(steps): 200000
- 입력 데이터 위치(dataset.repo_id): local/stacking_token_dataset
- 출력 데이터 위치(output_dir): outputs/train/stacking_token
- 작업 이름(job_name): act_stacking_token
...
```sh
lerobot-train \
  --config_path=outputs/train/stacking_token/checkpoints/last/pretrained_model/train_config.json \
  --resume=true
```
...
학습된 checkpoint/model이 준비되었다면 생성된 모델을 이용하여 로봇을 제어합니다. 로봇이 실행 중 과하게 움직이거나 멈춰야 할 때에는 `inference_node`를 실행 중인 터미널에서 `Ctrl+C`를 눌러 노드 실행을 중단할 수 있습니다.
...
- `policy_path`는 `outputs/train/stacking_token/checkpoints/last/pretrained_model`을 가리켜야 합니다.
