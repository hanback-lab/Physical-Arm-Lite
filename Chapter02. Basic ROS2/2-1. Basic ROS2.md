...
도메인은 터미널에서 `export`를 통해 변경할 수 있습니다. 도메인 ID가 다르면 DDS discovery 범위가 분리되어 서로의 node/topic을 발견하지 못합니다. 같은 실험실에 여러 대의 PC나 로봇이 있을 때 도메인을 다르게 하여 구분합니다. 아래 명령어를 통해 도메인을 원하는 번호로 변경할 수 있습니다.
...
Node Discovery란 네트워크에 있는 노드들이 서로를 자동으로 찾아내는 기능입니다. 예를 들어 PC A에서 `camera_node`를 통해 이미지 topic을 발행한다면, PC B에서 `camera_node`의 IP나 포트를 모르더라도 topic의 존재를 확인할 수 있습니다. 다만, 네트워크와 도메인 ID가 동일해야 합니다.
...
Windows에 ROS2를 설치하기를 원치 않는다면 SSH를 이용하여 원격접속을 할 수 있습니다. Visual Studio Code의 Extension을 활용하겠습니다. 'Remote-SSH'를 검색하고 install 버튼을 누릅니다. 
...
바로가기를 연 후, Display number를 0으로 바꿔쓰고 다음으로 넘어갑니다.
...
설정을 완료한 후 VSCode에서도 환경변수를 설정해야 합니다. Windows PC에서 cmd를 키고, 'ipconfig'를 입력해 ip를 확인합니다.
