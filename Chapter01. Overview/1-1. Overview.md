...
Physical Arm Lite는 현대 로봇공학과 Physical AI의 흐름을 반영하여 설계된 교육용 로보틱스 플랫폼입니다. 학습자는 이 장비를 통해 기초 로봇공학의 핵심 개념을 이해하고, 시뮬레이션 기반 검증과 VLA 기초 활용까지 연결된 실습을 수행합니다. 본 교재는 개별 공학 요소를 깊이 있게 다루는 것보다, 로봇 시스템을 구성하는 기술들이 어떻게 맞물려 동작하는지를 이해하고 경험하는 데 중점을 둡니다. 따라서 Physical Arm Lite는 기초 이론, 시스템 통합, 그리고 최신 지능형 로봇 기술의 흐름을 하나의 학습 경로로 이어 주는 출발점이 됩니다. 
...
제품에 탑재된 지능형 로봇 제어기는 **Jetson Orin Nano**입니다. 이 제어기는 아래와 같은 하드웨어 성능을 갖추고 있습니다.
...
PhysicAI Arm을 개발은 local 환경과 remote PC 환경 두 가지로 진행할 수 있습니다. Remote PC를 통한 개발을 하고 싶을 경우 [아래 Remote PC 환경 시작하기](/Chapter01.%20Overview/1-1.%20Overview.md#Remote-PC-환경-시작하기)부분을 읽으시면 됩니다. 바로 밑은 local 환경 개발에 대한 설명을 진행하겠습니다.

## local 환경 시작하기

Physical Arm Lite를 시작하기 위해선 전원 케이블을 연결한 후, 전원을 켜야 합니다. 본 제품의 측면을 보시면 포트들과 전원 버튼이 있는 것을 확인할 수 있습니다. 사진에 표시된 것과 같이 1번 포트에 전원 선을 연결합니다. 그 후, 2번의 전원 버튼을 0에서 1로 바꿔 전원을 켭니다. 제공된 키보드와 조이스틱은 3번 USB 포트에 각각 꽂습니다.
...
Node에 대한 자세한 설명은 2장에서 이어가도록 하겠습니다.
...
VSCode에서는 확장 기능(Extensions)을 제공합니다. 이 기능은 VSCode를 VSCode의 기능을 확장하여 **개인만의 개발 환경**을 만들 수 있는 플러그인입니다. 언어 지원, 디버깅, 코드 자동 완성, 협업 도구 등 다양한 기능을 추가할 수 있으며, 필요한 기능만을 선택적으로 설치해 효율적인 개발 환경을 구성합니다.
...
git clone -b busan https://github.com/hanback-lab/PhysicAI-Arm-ROS physicai_arm
cd physicai_arm
...
본 제품에는 Follower Arm, 즉 매니퓰레이터가 1대 포함되어 있습니다. 아래 명령어를 통해 조이스틱 제어를 활성화할 수 있습니다. 조이스틱 제어를 활성화하기 전, `bringup.launch.py` 런치 파일을 먼저 실행시켜주어야 합니다. 매니퓰레이터는 같이 제공된 조이스틱으로 조작이 가능하며 조작법은 다음과 같습니다.
