...
OpenCV는 다양한 색상 공간으로 표현하며, 이미지 처리 목적에 따라 **여러 색상 공간 간의 변환**을 지원합니다. BGR 이미지를 여러 색상 필터로 변환하고 채도 조절과 색상 마스킹을 실습합니다. 적용된 색상 필터는 총 20가지이며, `n`/`p`키를 이용하여 다음/이전 효과, `r`키로 새 이미지 생성, `s`/`q`키로 각각 결과 저장/종료를 할 수 있습니다.
...
def make_image(seed=42):
    ...
    for _ in range(6):
        ...
        color = [int(v) for v in np.random.randint(40, 220, 3)]
        cv2.rectangle(img, (x1, y1), (min(x2,639), min(y2,479)), color, -1)

    for _ in range(8):
        cx, cy = np.random.randint(50, 590), np.random.randint(50, 430)
        r = np.random.randint(25, 80)
        color = [int(v) for v in np.random.randint(40, 220, 3)]
        cv2.circle(img, (cx, cy), r, color, -1)

    pts_list = []
    for _ in range(4):
        cx, cy = np.random.randint(80, 560), np.random.randint(80, 400)
        pts = []
        n = np.random.randint(3, 7)
        for k in range(n):
            angle = 2 * math.pi * k / n + np.random.uniform(0, 0.5)
        ...
        color = [int(v) for v in np.random.randint(40, 220, 3)]
        cv2.fillPoly(img, [pts_list[-1]], color)

    img = cv2.GaussianBlur(img, (3, 3), 0)
    return img

def apply_color_effect(img, mode, params):
    if mode == "original":
        return img.copy()
        ...
        shift = params.get("hue_shift", 90)
        hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV).astype(np.int32)
        hsv[:, :, 0] = (hsv[:, :, 0] + shift) % 180
        return cv2.cvtColor(hsv.astype(np.uint8), cv2.COLOR_HSV2BGR)

    elif mode == "saturation":
        scale = params.get("sat_scale", 2.0)
        hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV).astype(np.float32)
        hsv[:, :, 1] = np.clip(hsv[:, :, 1] * scale, 0, 255)
        return cv2.cvtColor(hsv.astype(np.uint8), cv2.COLOR_HSV2BGR)

    elif mode == "brightness":
        beta = params.get("beta", 60)
...
MODES = [
    ...
    ("invert",         "Invert",                {}),
    ("sepia",          "Sepia",                 {}),
    ("hsv_shift",      "Hue Shift +90",         {"hue_shift": 90}),
    ("hsv_shift",      "Hue Shift +150",        {"hue_shift": 150}),
    ("saturation",     "Saturation x2.5",       {"sat_scale": 2.5}),
    ("saturation",     "Saturation x0.3",       {"sat_scale": 0.3}),
    ("brightness",     "Brightness +80",        {"beta": 80}),
    ("brightness",     "Brightness -80",        {"beta": -80}),
    ("contrast",       "Contrast x2.5",         {"alpha": 2.5}),
    ("gamma",          "Gamma 0.4 (Brighten)",  {"gamma": 0.4}),
    ("gamma",          "Gamma 2.5 (Darken)",    {"gamma": 2.5}),
    ("channel_swap",   "Channel Swap R→G→B",    {}),
    ("channel_filter", "Blue Channel Only",     {"channel": 0}),
    ("channel_filter", "Green Channel Only",    {"channel": 1}),
    ("channel_filter", "Red Channel Only",      {"channel": 2}),
    ("colormap",       "Colormap: JET",         {"cmap": cv2.COLORMAP_JET}),
    ("colormap",       "Colormap: MAGMA",       {"cmap": cv2.COLORMAP_MAGMA}),
    ("colormap",       "Colormap: OCEAN",       {"cmap": cv2.COLORMAP_OCEAN}),
    ("threshold_color","Threshold Highlight",   {"thresh": 100}),
]
...
print("[n/p] next/previous  [r] regen  [s] save  [q] quit")

while True:
    mode, label, params = MODES[idx]

    if mode == "channel_swap":
        b, g, r = cv2.split(img)
        params = {"order": [r, b, g]}

    result = apply_color_effect(img, mode, params)

    h, w = img.shape[:2]
    divider = np.full((h, 6, 3), 60, dtype=np.uint8)
    combined = np.hstack([img, divider, result])

    bar = np.zeros((52, combined.shape[1], 3), dtype=np.uint8)
    cv2.putText(bar, "Generated Image", (w // 2 - 80, 22),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (160, 160, 160), 1)
    cv2.putText(bar, label, (w + 6 + w // 2 - len(label) * 7, 22),
                cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 220, 180), 1)
    nav = f"[{idx+1}/{len(MODES)}]  n:next  p:prev  r:regen  s:save  q:quit"
    cv2.putText(bar, nav, (10, 44), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (100, 100, 100), 1)

    view = np.vstack([bar, combined])
    ...
    if key == ord('q'):
        ...
        print(f"new image generate (seed={seed})")
    elif key == ord('s'):
        fname = f"color_effect_{label.replace(' ', '_').replace(':', '')}.jpg"
...
이미지 필터링은 이미지의 **각 픽셀 주변값들을 수학적으로 연산하여 새로운 픽셀값을 만드는 기법**입니다. 블러, 샤프닝, 엠보싱 등의 필터가 있습니다. 임의의 이미지를 생성해서 필터링을 적용하겠습니다. 좌측에는 필터링 이전 이미지, 우측에는 필터링 적용 이미지가 표시되도록 구성되어 한눈에 비교할 수 있습니다. `n`/`p`를 누르면 각각 이전 필터/다음 필터를 확인할 수 있습니다. `s`키를 통해 결과를 저장할 수 있고 `q`를 누르면 종료됩니다.
...
def make_base_image():
    ...
    cv2.ellipse(img, (150, 380), (100, 40), 20, 0, 360, (180, 140, 50), -1)
...
kernels = {
    "blur_3":   ("Gaussian Blur 3x3",  lambda i: cv2.GaussianBlur(i, (3, 3), 0)),
    "blur_15":  ("Gaussian Blur 15x15", lambda i: cv2.GaussianBlur(i, (15, 15), 0)),
    "median":   ("Median Blur",         lambda i: cv2.medianBlur(i, 7)),
    "bilateral":("Bilateral Filter",    lambda i: cv2.bilateralFilter(i, 9, 75, 75)),
    "sharpen":  ("Sharpen",             lambda i: cv2.filter2D(i, -1, np.array([
                                            [-1, -1, -1],
                                            [-1,  9, -1],
                                            [-1, -1, -1]]))),
    "emboss":   ("Emboss",              lambda i: cv2.filter2D(i, -1, np.array([
                                            [-2, -1,  0],
                                            [-1,  1,  1],
                                            [ 0,  1,  2]]))),
    "edge_lap": ("Laplacian Edge",      lambda i: cv2.convertScaleAbs(cv2.Laplacian(
                                            cv2.cvtColor(i, cv2.COLOR_BGR2GRAY), cv2.CV_64F))),
    "sobel_x":  ("Sobel X",            lambda i: cv2.convertScaleAbs(cv2.Sobel(
                                            cv2.cvtColor(i, cv2.COLOR_BGR2GRAY), cv2.CV_64F, 1, 0, ksize=3))),
}

key_list = list(kernels.keys())
idx = 0

print("[n] next filter  [p] previous filter  [s] save  [q] quit")

while True:
    key_name = key_list[idx]
    label, fn = kernels[key_name]
    result = fn(img)
    ...
    combined[:, :w] = img
    combined[:, w+10:] = result_bgr

    bar = np.zeros((40, w * 2 + 10, 3), dtype=np.uint8)
    cv2.putText(bar, "Original", (w // 2 - 50, 28),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (180, 180, 180), 1)
    cv2.putText(bar, label, (w + 10 + w // 2 - len(label) * 7, 28),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 220, 180), 1)
    nav = f"[{idx+1}/{len(key_list)}]  n:next  p:prev  s:save  q:quit"
    cv2.putText(bar, nav, (10, 16), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (120, 120, 120), 1)

    view = np.vstack([bar, combined])
    cv2.imshow("Image Filtering", view)
    ...
    if key == ord('q'):
        ...
        fname = f"filter_{key_name}.jpg"
        cv2.imwrite(fname, result_bgr)
        print(f"저장: {fname}")
...
템플릿 매칭은 **큰 이미지 안에서 작은 이미지와 가장 유사한 위치를 찾는 기법**입니다. 템플릿을 씬 위에 슬라이딩하여 유사도를 계산하고 유사도가 가장 높은 위치를 결과로 반환합니다. 임의의 이미지를 생성하여 해당 이미지 안에서 특정 패턴의 위치를 찾습니다. `1`/`2`/`3`키를 눌러 매칭 방식을 변경하고 `m`키는 단일↔멀티 매칭 모드 변경, `+`/`-`키는 임계값 조정, `s`키는 결과 저장, `q`키는 종료입니다.
...
def make_scene():
    ...
    for _ in range(12):
        ...
        c = [int(v) for v in np.random.randint(80, 200, 3)]
        cv2.circle(scene, (x, y), r, c, -1)

    cv2.rectangle(scene, (150, 130), (210, 190), (50, 80, 200), -1)
    cv2.rectangle(scene, (380, 260), (440, 320), (50, 80, 200), -1)
    cv2.rectangle(scene, (510, 80),  (570, 140), (50, 80, 200), -1)
    cv2.rectangle(scene, (90,  330), (150, 390), (50, 80, 200), -1)

    for x1, y1, x2, y2 in [(150,130,210,190),(380,260,440,320),(510,80,570,140),(90,330,150,390)]:
        cv2.rectangle(scene, (x1, y1), (x2, y2), (30, 50, 160), 2)
        mx, my = (x1+x2)//2, (y1+y2)//2
        cv2.line(scene, (mx-10, my), (mx+10, my), (220, 220, 255), 2)
        cv2.line(scene, (mx, my-10), (mx, my+10), (220, 220, 255), 2)

    noise = np.random.randint(0, 15, scene.shape, dtype=np.uint8)
    scene = cv2.add(scene, noise)
    return scene

def make_template(scene):
    x1, y1, x2, y2 = 150, 130, 210, 190
    tmpl = scene[y1:y2, x1:x2].copy()
    tmpl = cv2.GaussianBlur(tmpl, (3, 3), 0)
    return tmpl

print("scene.jpg / template.jpg 없음 → 생성 이미지 사용")
scene_orig = make_scene()
tmpl_orig  = make_template(scene_orig)

METHODS = {
    ord('1'): (cv2.TM_CCOEFF_NORMED,  "TM_CCOEFF_NORMED  [1]"),
    ord('2'): (cv2.TM_CCORR_NORMED,   "TM_CCORR_NORMED   [2]"),
    ord('3'): (cv2.TM_SQDIFF_NORMED,  "TM_SQDIFF_NORMED  [3]"),
}
current_method = cv2.TM_CCOEFF_NORMED
current_label  = "TM_CCOEFF_NORMED  [1]"
threshold = 0.75
multi_mode = False

def run_matching(scene, tmpl, method, thresh, multi):
    result = cv2.matchTemplate(scene, tmpl, method)
    min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)

    vis = scene.copy()
    th, tw = tmpl.shape[:2]

    score_map = cv2.normalize(result, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
    score_bgr = cv2.applyColorMap(score_map, cv2.COLORMAP_JET)
    score_bgr = cv2.resize(score_bgr, (scene.shape[1], scene.shape[0]))

    if method == cv2.TM_SQDIFF_NORMED:
        best_loc = min_loc
        best_score = 1.0 - min_val
    else:
        best_loc = max_loc
        best_score = max_val
    ...
    if multi:
        if method == cv2.TM_SQDIFF_NORMED:
            locs = np.where(result <= (1.0 - thresh))
        else:
        ...
        boxes = np.array(boxes) if boxes else np.empty((0, 4), dtype=int)
        kept = []
        if len(boxes):
            scores_list = []
            for b in boxes:
            ...
            indices = cv2.dnn.NMSBoxes(
                boxes.tolist(), scores_list, score_threshold=0.0, nms_threshold=0.3
            )
            if len(indices):
        ...
        for b in kept:
            cv2.rectangle(vis, (b[0], b[1]), (b[2], b[3]), (0, 220, 80), 2)
        count = len(kept)
    else:
        x, y = best_loc
        cv2.rectangle(vis, (x, y), (x+tw, y+th), (0, 220, 80), 2)
        cv2.circle(vis, (x + tw//2, y + th//2), 5, (0, 255, 255), -1)
        count = 1

    return vis, score_bgr, best_score, count

print("[1/2/3] matching template  [m] multi mode  [+/-] threshold  [s] save  [q] quit")

while True:
    vis, score_map, best_score, count = run_matching(
        scene_orig, tmpl_orig, current_method, threshold, multi_mode
    )

    th, tw = tmpl_orig.shape[:2]
    tmpl_pad = np.ones((scene_orig.shape[0], tw + 20, 3), dtype=np.uint8) * 230
    ty = (scene_orig.shape[0] - th) // 2
    tmpl_pad[ty:ty+th, 10:10+tw] = tmpl_orig
    cv2.putText(tmpl_pad, "Template", (5, ty - 10),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (80, 80, 80), 1)

    combined = np.hstack([vis, tmpl_pad, score_map])

    bar = np.zeros((50, combined.shape[1], 3), dtype=np.uint8)
    mode_str = "MULTI" if multi_mode else "SINGLE"
    info = (f"Method: {current_label}   Score: {best_score:.3f}   "
            f"Thresh: {threshold:.2f}   Mode: {mode_str}   Found: {count}")
    cv2.putText(bar, info, (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 210, 160), 1)
    cv2.putText(bar, "1/2/3:method  m:multi  +/-:thresh  s:save  q:quit",
                (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (120, 120, 120), 1)
    ...
    if key == ord('q'):
        ...
        current_method, current_label = METHODS[key]
    elif key == ord('m'):
        multi_mode = not multi_mode
        print(f"multi mode: {multi_mode}")
    elif key == ord('+') or key == ord('='):
        threshold = min(0.99, round(threshold + 0.05, 2))
        print(f"threshold: {threshold}")
    elif key == ord('-'):
        threshold = max(0.10, round(threshold - 0.05, 2))
        print(f"threshold: {threshold}")
    elif key == ord('s'):
        cv2.imwrite("matching_result.jpg", vis)
        print("save: matching_result.jpg")
