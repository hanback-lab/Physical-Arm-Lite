...
본 제품에 탑재된 지능형 로봇 제어기는 **Jetson Orin Nano**입니다. 본 지능형 로봇 제어기는 아래와 같은 하드웨어 성능을 갖추고 있습니다.
...
지능형 로봇 제어기에 탑재된 OS는 *Jetson Linux 36.3.0* 이며, 자사의 추가 설정으로 아래 모듈 및 소프트웨어가 탑재되어 있습니다.
...
6장, 7장을 제외한 모든 실습 및 예제는 아래 환경에서 동작합니다. 상단에 기재된 소프트웨어 버전을 **임의로 업그레이드하거나 다운그레이드**할 경우 실습이 **원활히 진행되지 않을 수 있습니다.** 반드시 이 점 숙지해 주시기 바랍니다.

### 시작하기

Physical Arm Lite를 시작하기 위해선 콘센트 선을 연결한 후, 전원을 켜야 합니다. 본 제품의 측면을 보시면 포트들과 전원 버튼이 있는 것을 확인할 수 있습니다. 사진에 표시된 것과 같이 1번 포트에 전원 선을 연결해줍니다. 그 후, 2번의 전원 버튼을 0에서 1로 바꿔 전원을 켜줍니다. 제공된 키보드와 조이스틱은 3번 USB 포트에 각각 꽂습니다.
...
본 제품에는 Follower Arm(이하 매니퓰레이터)이 하나로 구성되어 있습니다. 매니퓰레이터는 같이 제공된 조이스틱으로 조작이 가능하며 조작법은 다음과 같습니다.
...
장비에 탑재된 지능형 로봇 제어기에는 이미 VSCode가 설치되어 있습니다. 아래 사진과 같이 왼쪽 사이드바의 실행 아이콘을 클릭하거나, 새 터미널을 연 뒤 'code'라는 명령어를 입력하여 VSCode를 실행합니다.
