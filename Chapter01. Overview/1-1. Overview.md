...
본 제품에 탑재된 워크셀 베이스는 **Jetson Orin Nano**입니다. 본 워크셀 베이스는 아래와 같은 하드웨어 성능을 갖추고 있습니다.
...
워크셀 베이스에 탑재된 OS는 *Jetson Linux 36.3.0* 이며, 자사의 추가 설정으로 아래 모듈 및 소프트웨어가 탑재되어 있습니다.
...
장비에 탑재된 워크셀 베이스에는 이미 VSCode가 설치되어 있습니다. 아래 사진과 같이 왼쪽 사이드바의 실행 아이콘을 클릭하거나, 새 터미널을 연 뒤 'code'라는 명령어를 입력하여 VSCode를 실행합니다.
