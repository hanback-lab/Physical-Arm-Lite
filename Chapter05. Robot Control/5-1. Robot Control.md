...
앞서 배운 `ros2 topic echo` 명령어가 아닌 Python을 이용하여 관절값을 저장해보겠습니다. JointState 토픽값을 구독하여 joint_log.csv 파일로 저장하는 예제입니다.
...
앞서 배운 `ros2 topic echo` 명령어가 아닌 Python을 이용하여 ee_pose를 기록해보겠습니다. ee_pose 토픽값을 구독하여 Matplotlib를 활용해 ee_pose의 경로를 그래프로 시각화하겠습니다. 다음 예제는 ee_pose를 10초동안 저장한 뒤, 시각화하는 실습입니다.
...
class EEPoseLogger(Node):
    def __init__(self):
        ...
        self.get_logger().info("Recording /ee_pose for 10 seconds...")
    ...
    def callback(self, msg):
        ...
        self.get_logger().info(
            f"t={elapsed:.2f}s | x={x:.3f}, y={y:.3f}, z={z:.3f}"
        )

    def plot_result(self):
        plt.figure(figsize=(8, 5))
        plt.plot(self.times, self.x_values, label="x")
        plt.plot(self.times, self.y_values, label="y")
        plt.plot(self.times, self.z_values, label="z")

        plt.xlabel("Time (s)")
        plt.ylabel("Position (m)")
        plt.title("EE Pose Position over Time")
        plt.grid(True)
        plt.legend()
        plt.show()
...
class SafeJointTargetPublisher(Node):
    def __init__(self):
        ...
        self.timer = self.create_timer(1.0, self.publish.target)
